...
export function MobileShell({ children }) {
  const { viewportSettings, setViewportSettings, startOnboarding } = useApp();
  const { isEmulating, width } = viewportSettings;

  // Real mobile / PWA detection (default to true to prevent notch flash on real devices)
  const [isRealMobile, setIsRealMobile] = useState(true);
  ...
  const showFrame = !isRealMobile && isEmulating;
  const currentHour = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

  return (
    <div className={`min-h-screen font-sans flex flex-col items-center justify-start ${showFrame ? 'bg-slate-950 text-slate-100 sm:p-4' : 'bg-slate-50 text-slate-900 w-full p-0'}`}>
      {/* Top Testing Toolbar (Only visible on desktop screens >= 768px) */}
      {!isRealMobile && (
      ...
      <main
        className={`
          relative transition-all duration-300 w-full overflow-hidden flex flex-col flex-1
          ${showFrame 
            ? 'rounded-[44px] shadow-[0_25px_70px_rgba(0,0,0,0.65)] border-[8px] border-slate-800 ring-1 ring-slate-700 my-auto' 
            : 'max-w-md min-h-screen shadow-none border-none rounded-none mx-auto'
          }
        `}
        style={{
          width: showFrame ? `${width}px` : '100%',
          maxWidth: showFrame ? `${width}px` : '430px',
          minHeight: showFrame ? '844px' : '100vh',
          height: showFrame ? '844px' : 'auto',
          backgroundColor: '#F8FAFC',
          color: '#0F172A',
        }}
      >
        {/* Smartphone Notch / Dynamic Island: ONLY on desktop when simulated frame is active */}
        {showFrame && (
        ...
        <div className={`h-full flex-1 overflow-y-auto flex flex-col ${showFrame ? 'pt-6' : ''}`}>
          {children}
        </div>
      </main>
    </div>
